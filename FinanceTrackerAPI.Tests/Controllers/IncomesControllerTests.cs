...
public class IncomesControllerTests
    {
        ...
        public async Task GetIncomes_ReturnsOkResultWithIncomes()
        {
            ...
            var returnValue = Assert.IsType<object>(okResult.Value); // We return an anonymous object with Items, TotalCount, etc.
            var itemsProperty = returnValue.GetType().GetProperty("Items");
            var incomes = Assert.IsAssignableFrom<IEnumerable<Income>>(itemsProperty?.GetValue(returnValue));
        ...
        public async Task GetIncome_WithValidId_ReturnsOkResultWithIncome()
        {
            ...
            Assert.IsType<Income>(okResult.Value);
        ...
        public async Task PostIncome_WithValidModel_ReturnsCreatedAtActionResult()
        {
            // Arrange
            using var context = GetInMemoryDbContext();
            var income = new Income { Description = "Investment", Amount = 100, Date = DateTime.Now };
            var controller = new IncomesController(context);

            // Act
            var result = await controller.PostIncome(income);
            var createdAtActionResult = Assert.IsType<CreatedAtActionResult>(result.Result);
            var createdIncome = Assert.IsType<Income>(createdAtActionResult.Value);

            // Assert
            Assert.Equal(income.Description, createdIncome.Description);
        ...
        public async Task PostIncome_WithInvalidModel_ReturnsBadRequestResult()
        {
            // Arrange
            using var context = GetInMemoryDbContext();
            var income = new Income { Amount = -100 }; // Invalid amount
            var controller = new IncomesController(context);
        ...
        public async Task PutIncome_WithValidIdAndModel_ReturnsNoContentResult()
        {
            ...
            await context.SaveChangesAsync();
            var updatedIncome = new Income { Id = 1, Description = "New Salary", Amount = 2500, Date = DateTime.Now };
            var controller = new IncomesController(context);

            // Act
            var result = await controller.PutIncome(1, updatedIncome);
        ...
        public async Task PutIncome_WithInvalidId_ReturnsBadRequestResult()
        {
            ...
            // Act
            var result = await controller.PutIncome(99, income);
        ...
        public async Task PutIncome_WithInvalidModel_ReturnsBadRequestResult()
        {
            // Arrange
            using var context = GetInMemoryDbContext();
            var income = new Income { Id = 1, Amount = -100 }; // Invalid amount
            var controller = new IncomesController(context);
            controller.ModelState.AddModelError("Amount", "Amount must be positive.");

            // Act
            var result = await controller.PutIncome(1, income);
